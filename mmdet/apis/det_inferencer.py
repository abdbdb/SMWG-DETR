...
class DetInferencer(BaseInferencer):
    ...
    def __call__(
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            def build_filename_to_id_map(coco):
                img_to_id = {}
                for img in coco.dataset['images']:
                    img_to_id[img['file_name']] = img['id']
                return img_to_id

            def visualize_detections(image_path, predictions, coco, filename_to_id_map, save_path):
                file_name = os.path.basename(image_path)
                img_id = filename_to_id_map[file_name]
                ann_ids = coco.getAnnIds(imgIds=[img_id])
                annotations = coco.loadAnns(ann_ids)
                gt_boxes = [ann['bbox'] for ann in annotations]  # COCO's bbox format is [x, y, width, height]
                gt_boxes = [[x, y, x + w, y + h] for x, y, w, h in gt_boxes]  # Convert to [x1, y1, x2, y2]
                gt_label = [ann['category_id'] for ann in annotations]

                image = cv2.imread(image_path)
                image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

                # 设置图像大小与显示
                fig, ax = plt.subplots(figsize=(12, 12), dpi=600)  # 设置高 DPI
                ax.imshow(image)
                ax.axis('off')  # 不显示坐标轴

                # 处理预测框与 GT 框
                filtered_preds = [(bbox, score, label) for bbox, score, label in zip(
                    predictions['bboxes'], predictions['scores'], predictions['labels']) if score > 0.3]
                ...
                for index, gt_box in enumerate(gt_boxes):
                    matched = False
                    for i, (bbox, score, label) in enumerate(filtered_preds):
                        if compute_iou(gt_box, bbox) > 0.2:

                            if gt_label[index] == label:
                                color = 'g'
                            else:
                                color = 'g'
                            rect = patches.Rectangle((gt_box[0], gt_box[1]), gt_box[2] - gt_box[0],
                                                     gt_box[3] - gt_box[1], linewidth=1.5, edgecolor=color,
                                                     facecolor='none')
                            ax.add_patch(rect)
            ...
            # Example usage
            coco = COCO(ann_path)  # Initialize the COCO object
            filename_to_id_map = build_filename_to_id_map(coco)

            for img_path, prediction in zip(ori_imgs, results['predictions']):
                save_path_each = os.path.join(save_path, os.path.basename(img_path))
                visualize_detections(img_path, prediction, coco, filename_to_id_map, save_path_each)
