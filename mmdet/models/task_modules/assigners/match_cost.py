...
@ TASK_UTILS.register_module()
class GaussianCost(BaseMatchCost):
    ...
    def __call__(self,
        ...
        gt_bboxes = bbox_xyxy_to_cxcywh(gt_bboxes) / 800
        pred_bboxes = bbox_xyxy_to_cxcywh(pred_bboxes) / 800

        overlaps = self.cau_gaussian_cost(gt_bboxes, pred_bboxes, self.tau, self.f_x).t()

        if fp16:
            overlaps = overlaps.to(torch.float16)

        # The 1 is a constant that doesn't change the matching, so omitted.
        gaussian_cost = overlaps

        return gaussian_cost * self.weight

    def cau_gaussian_cost(self, gt, box, tau, f_x):
