...
class DeformableDETR(DetectionTransformer):
    ...
    def pre_transformer(
        ...
        for lvl, (feat, mask, pos_embed) in enumerate(
            ...
            lvl_pos_embed = pos_embed + self.level_embed[lvl].view(1, 1, -1)  # 位置编码由余弦编码（应该是）+特征层编码构成
    ...
    def forward_encoder(self, feat: Tensor, feat_mask: Tensor,
        ...
        """
        memory = self.encoder(
            query=feat,  # 展开后的多尺度特征图
            query_pos=feat_pos,  # 展开后的位置编码
    ...
    def gen_encoder_output_proposals(
        ...
        for lvl, HW in enumerate(spatial_shapes):
            ...
            if memory_mask is not None:
                ...
                scale = HW.unsqueeze(0).flip(dims=[0, 1]).view(1, 1, 1, 2)  # filp反转输入张量的维度
            ...
            grid = (grid.unsqueeze(0).expand(bs, -1, -1, -1) + 0.5) / scale   # 这里确定锚框的尺度
        ...
        # inverse_sigmoid
        output_proposals = torch.log(output_proposals / (1 - output_proposals))  # 反sigmoid，转到非归一化空间，网络比较好学习，之后通过sigmoid可以重新转换到归一化空间
        if memory_mask is not None:
