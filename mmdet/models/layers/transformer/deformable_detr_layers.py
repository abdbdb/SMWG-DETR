...
class DeformableDetrTransformerEncoder(DetrTransformerEncoder):
    ...
    def _init_layers(self) -> None:
        ...
        self.hold_result = 1  # 这个变量是用来保存中间结果的，实际运行的时候可以删掉
    ...
    def forward(self, query: Tensor, query_pos: Tensor,
        ...
        # 添加保存中间变量的代码，这一块是backbone提取的原始特征
        # with open('D:\\code\\my_code\\tint-detect-mm3.3-result\\vis_encoder_and_decoder\\encoder_output\\' + 'query_'
        #           + str(0) + '.pkl', 'wb') as f:
        #     pickle.dump(query.detach().cpu().numpy(), f)
        ...
        for layer in self.layers:
            ...
            # 添加保存中间变量的代码
            # with open('D:\\code\\my_code\\tint-detect-mm3.3-result\\vis_encoder_and_decoder\\encoder_output\\'+'query_'
            #           + str(self.hold_result) + '.pkl', 'wb') as f:
            #     pickle.dump(query.detach().cpu().numpy(), f)
            self.hold_result += 1
