...
def mal_loss(pred: Tensor,
             target: Tensor,
             one_hot_target: Tensor,
             weight: Optional[Tensor] = None,
             alpha: float = 0.75,
             gamma: float = 2.0,
             iou_weighted: bool = False,
             reduction: str = 'mean',
    ...
    target = target.type_as(pred)  # 这个相当于是IOU

    if iou_weighted:
        focal_weight = target * (target > 0.0).float() + \
                       alpha * pred_sigmoid.abs().pow(gamma) * \
                       (target <= 0.0).float()
    else:
        target = target.pow(gamma)
        focal_weight = 1 * (one_hot_target > 0.0).float() + \
                       alpha * pred_sigmoid.abs().pow(gamma) * \
                       (one_hot_target <= 0.0).float()
